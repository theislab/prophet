...
def dataloader_phenotypes(
    ...
    gene_embedding = pd.concat(gene_embedding) if isinstance(gene_embedding, list) else gene_embedding
    cell_lines_embedding = pd.concat(cell_lines_embedding) if isinstance(cell_lines_embedding, list) else cell_lines_embedding
    phenotype_embedding = pd.concat(phenotype_embedding) if isinstance(phenotype_embedding, list) and phenotype_embedding[0] is not None else (phenotype_embedding[0] if isinstance(phenotype_embedding, list) else phenotype_embedding)
