...
class DataValidator:
    ...
    def validate_dataframe(
        ...
        if df.duplicated().any():
            issues["other_issues"].append(f"Found {df.duplicated().sum()} duplicate rows")
    ...
    def print_validation_report(issues: Dict[str, List[str]]) -> None:
        ...
        if any("Missing values" in str(issue) for issue in issues["other_issues"]):
            print("  - Handle missing values using DataProcessor.handle_missing_values()")
