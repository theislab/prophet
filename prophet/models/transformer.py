...
class TransformerPredictor(pl.LightningModule):
    ...
    def _create_model(self):
        ...
        self.transformer = nn.TransformerEncoder(
            encoder_layer=layer, num_layers=self.hparams.num_layers
        )

        # 2 layers regressor
        dim_regressor_input = (
            2 * self.hparams.model_dim + self.learnable_embedding.embedding_dim
        )  # CLS (model_dim) | CellLine (model_dim) | phenotype (varies)
        if not self.hparams.simpler:
            dim_regressor_input = self.hparams.model_dim
        if self.hparams.sum:
            dim_regressor_input = (
                dim_regressor_input + self.hparams.model_dim + self.hparams.model_dim
            )
        ...
        if self.hparams.linear_predictor:
            self.output_net = nn.Sequential(nn.Linear(dim_regressor_input, 1))
    ...
    def forward(self, phenotype, cl, perturbations, perturbations_type, attn_mask):
        ...
        cl_embedding = self.cl_net(cl).unsqueeze(
            1
        )  # unsqueeze just useful if not simpler
        phenotype_emb = phenotype_emb.unsqueeze(1)
        # bs x n x dim

        # Regression token (CLS) stored in index 0
        cls = torch.zeros(
            size=(phenotype_emb.shape[0], 1),
            device=phenotype_emb.device,
            dtype=torch.int32,
        )
        cls = self.learnable_embedding(
            cls
        )  # we get the embedding, shape (bs x 1 x dim)

        if self.hparams.pool == "cls":
            if (
                self.hparams.simpler
            ):  # if simpler, just perturbations and CLS to transformer
                embeddings = torch.cat((cls, perturbations), dim=1)
            else:  # otherwise everuthing to transformer
                embeddings = torch.cat(
                    (cls, perturbations, cl_embedding, phenotype_emb), dim=1
                )
            ...
            if self.hparams.simpler:
                ...
                embeddings = torch.cat(
                    (perturbations, cl_embedding, phenotype_emb), dim=1
                )
        # Run Transformer Layer
        if not self.hparams.sum:
            if self.hparams.mask:
                x = self.transformer(
                    embeddings, mask=None, src_key_padding_mask=attn_mask
                )
            else:
        ...
        if self.hparams.simpler:
            x = torch.cat(
                (x, cl_embedding.squeeze(1), phenotype_emb.squeeze(1)), dim=-1
            )
    ...
    def embedding(self, phenotype, cl, perturbations, perturbations_type, attn_mask):
        ...
        if self.hparams.simpler:
            x = torch.cat(
                (pert_emb, cl_embedding.squeeze(1), phenotype_emb.squeeze(1)), dim=-1
            )

        x = self.output_net(x)

        return {
            "pert_emb": pert_emb,  # output of transformer: CLS or mean
            "output": x,
    ...
    def get_embeddings(self, batch):
        ...
        return {
            "pert_emb": emb_dict["pert_emb"],
            "output": emb_dict["output"],
            "perturbations_after_transformer": emb_dict[
                "perturbations_after_transformer"
            ],
...
def load_models_config(models_config, seed, hparams=False, trial=None, phenotypes=None):
    ...
    transformer = TransformerPredictor(
        dim_cl=models_config.transformer.dim_cl,
        dim_iv=models_config.transformer.dim_iv,
        dim_phe=models_config.transformer.dim_phe,
        model_dim=models_config.transformer.model_dim,
        num_heads=models_config.transformer.num_heads,
        num_layers=models_config.transformer.num_layers,
        iv_dropout=models_config.transformer.iv_dropout,
        cl_dropout=models_config.transformer.cl_dropout,
        ph_dropout=models_config.transformer.ph_dropout,
        regressor_dropout=models_config.transformer.regressor_dropout,
        lr=models_config.transformer.lr,
        weight_decay=models_config.transformer.weight_decay,
        warmup=models_config.transformer.warmup,
        batch_size=models_config.batch_size,
        max_iters=models_config.transformer.max_iters,
        dropout=models_config.transformer.dropout,
        pool=models_config.transformer.pool,
        simpler=models_config.transformer.simpler,
        ctx_len=models_config.ctx_len,
        mask=models_config.transformer.mask,
        sum=models_config.transformer.sum,
        explicit_phenotype=models_config.transformer.explicit_phenotype,
        linear_predictor=models_config.transformer.linear_predictor,
        tokenizer_layers=models_config.transformer.tokenizer_layers,
        seed=seed,
        phenotypes=None,
    )
