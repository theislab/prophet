...
def prrc(targets, predictions, i):
    ...
    hitratio_top = (
        np.sum(actual_top & predicted_top) / np.sum(predicted_top)
        if np.sum(predicted_top) > 0
        else 0
    )
    hitratio_bottom = (
        np.sum(actual_bottom & predicted_bottom) / np.sum(predicted_bottom)
        if np.sum(predicted_bottom) > 0
        else 0
    )
...
def compute_hit_ratio(predictions, targets, cl, phenotypes, iv, topk, suffix=""):
    ...
    for i in topk:
        ...
        if all_cl_precisions_top:
            ...
            logging_dict[f"cl_average_precision_bottom_{i}{suffix}"] = (
                avg_cl_precision_bottom
            )
        ...
        if all_iv_precisions_top:
            ...
            logging_dict[f"iv_average_precision_bottom_{i}{suffix}"] = (
                avg_iv_precision_bottom
            )
...
class HitRatioCallback(pl.Callback):
    ...
    def _initialize_mappings(self, trainer, dataset_type="val"):
        ...
        if (
            dataset_type == "val"
            and hasattr(trainer, "val_dataloaders")
            and trainer.val_dataloaders
        ):
            dataset = trainer.val_dataloaders.dataset
        elif (
            dataset_type == "test"
            and hasattr(trainer, "test_dataloaders")
            and trainer.test_dataloaders
        ):
            dataset = (
                trainer.test_dataloaders[0].dataset
                if isinstance(trainer.test_dataloaders, list)
                else trainer.test_dataloaders.dataset
            )
        else:
            return False
    ...
    def _compute_hit_ratios(
        self, predictions, targets, phenotypes, dataset_indices, suffix=""
    ):
        ...
        for idx in dataset_indices:
            ...
            if self.index_to_ph_name and ph_idx in self.index_to_ph_name:
                ...
                ph_name = phenotypes[
                    len(cl_names)
                ]  # Use the phenotype from model output
