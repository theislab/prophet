...
def create_experimental_space(
    ...
    orientation: Literal["interventions_x_cells", "cells_x_interventions"] = "interventions_x_cells",
    ...
    if len(df) > len(df[group_cols].drop_duplicates()):
        ...
        df = df.groupby(group_cols)[prediction_col].agg(agg_funcs[aggregate_func]).reset_index()
    
    # Create pivot table
    pivot_df = df.pivot(
        index=intervention_col,
    ...
    if output_format == "dataframe":
        ...
        adata.uns["prophet_prediction"] = {
            ...
            "cell_line_col": cell_line_col,
            "prediction_col": prediction_col,
            "n_interventions": len(pivot_df.index) if orientation == "interventions_x_cells" else len(pivot_df.columns),
            "n_cell_lines": len(pivot_df.columns) if orientation == "interventions_x_cells" else len(pivot_df.index),
            "fill_missing": fill_missing,
            "aggregate_func": aggregate_func
        }
        ...
        raise ValueError(f"output_format must be 'dataframe' or 'anndata', got '{output_format}'")
...
def experimental_space_summary(
    ...
    if hasattr(experimental_space, 'X'):  # AnnData object
        ...
        print("=" * 50)
        print(f"Observations: {len(obs_names)} ({metadata.get('orientation', 'unknown').split('_x_')[0]})")
        print(f"Variables: {len(var_names)} ({metadata.get('orientation', 'unknown').split('_x_')[1]})")
        print(f"Matrix shape: {matrix.shape}")
        print(f"Data type: {matrix.dtype}")
        
        if not np.isnan(matrix).all():
            print(f"Value range: {np.nanmin(matrix):.3f} - {np.nanmax(matrix):.3f}")
            print(f"Missing values: {np.isnan(matrix).sum()} ({np.isnan(matrix).mean()*100:.1f}%)")
        else:
        ...
        if not np.isnan(values).all():
            ...
            print(f"Missing values: {np.isnan(values).sum()} ({np.isnan(values).mean()*100:.1f}%)")
        else:
            print("All values are NaN")
...
def interventions_x_cells(predictions_df: pd.DataFrame, **kwargs) -> Union[pd.DataFrame, "anndata.AnnData"]:
    """Create interventions × cell lines matrix (m × n)."""
    return create_experimental_space(
        predictions_df, 
        orientation="interventions_x_cells", 
        **kwargs
    )


def cells_x_interventions(predictions_df: pd.DataFrame, **kwargs) -> Union[pd.DataFrame, "anndata.AnnData"]:
    """Create cell lines × interventions matrix (n × m)."""
    return create_experimental_space(
        predictions_df,
        orientation="cells_x_interventions",
        **kwargs
    )
