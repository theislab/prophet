...
class DataFrameValidator:
    ...
    def validate_data_types(
        ...
        for col, expected_type in column_types.items():
            ...
            if expected_type is float and not pd.api.types.is_numeric_dtype(current_type):
                if convert_if_possible:
                    try:
                        ...
                        raise ValidationError(f"Cannot convert column '{col}' to numeric: {e}")
                else:
                    raise ValidationError(
...
class ModelValidator:
    ...
    def validate_model_config(
        ...
        if required_fields is None:
            required_fields = [
                'max_steps', 'batch_size', 'Transformer'
            ]
...
def _remove_nonexistent_categories(df: pd.DataFrame, 
                                 iv_embedding: dict, 
                                 cl_embedding: dict, 
                                 ph_embedding: Optional[dict],
                                 iv_col: Union[str, List[str]], 
                                 cl_col: str, 
                                 ph_col: str) -> pd.DataFrame:
    """
    ...
    if ph_embedding is not None:
        df_cleaned = remove_nonexistent_cat(df_cleaned, ph_embedding, ph_col, verbose=False)
...
def validate_prophet_inputs(
    ...
    if df is not None:
        ...
        # Process priors
        iv_embedding, cl_embedding, ph_embedding = process_priors(iv_emb_path, 
                                                                  cl_emb_path, 
                                                                  ph_emb_path)
        
        # Remove nonexistent categories using extracted function
        df_validated = _remove_nonexistent_categories(
            df_validated, iv_embedding, cl_embedding, ph_embedding,
            iv_col, cl_col, ph_col
        )
