...
parser.add_argument("--setting", type=str, default="Rad", required=False)
parser.add_argument(
    "--leaveout_method", type=str, default="leave_one_cl_out", required=False
)
parser.add_argument(
    "--config_file", type=str, required=True
)  # config file with info regarding the architecture
...
if __name__ == "__main__":
    ...
    data_label, gene_prior, cl_prior, phe_prior, path = get_data_by_setting(
        models_config.setting,
        models_config.genes_prior,
        models_config.cell_lines_prior,
        models_config.phenotype_prior,
    )
    ...
    for index in indices:
        ...
        train_transformer(
            data=data, model=model, config=models_config, name=index[-1], seed=seed
        )
        del data
        del model
