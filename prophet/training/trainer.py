...
class ProphetTrainer:
    ...
    def load_data(self, data_config: dict) -> pd.DataFrame:
        ...
        for path in tqdm(data_paths, desc="Loading datasets"):
            ...
            if "SCORE_dataset.csv" in path:
                df = df.sample(frac=0.33, replace=False, random_state=42).reset_index(
                    drop=True
                )
    ...
    def create_splits(
        ...
        data_label = pd.concat(
            [data_label, data_label_flipped], axis=0, ignore_index=True
        )
        data_label = data_label.reset_index(drop=True)
        ...
        def random_validation(train_indices, frac=0.2, seed=42):
        ...
        def unseen_cl_validation(indices, seed, n=3, data=data_label):
            ...
            if len(cls) <= n:
                print(
                    "Validation is random split because there are not enough cell lines left!"
                )
                return random_validation(indices, frac=0.2, seed=seed)
            else:
        ...
        def unseen_ivs_validation(indices, seed, n=3):
            ...
            ivs = list(
                sorted(
                    set(data_label.loc[indices].iv1.values)
                    & set(data_label.loc[indices].iv2.values)
                    - set(["negative_gene", "negative_drug"])
                )
            )
            ivs_exclude = _choose(ivs, n, seed=seed)
            valid_indices = data_label.index[
                data_label["iv1"].isin(ivs_exclude)
                | data_label["iv2"].isin(ivs_exclude)
            ].tolist()
            valid_indices = sorted(
                set(valid_indices) & set(indices)
            )  # validation must be selected from train
            indices = np.setdiff1d(indices, valid_indices)
            return indices, valid_indices
        ...
        def realign_indices(full_data, subset, indices):
            ...
            for index in indices:
                ...
                real_train_indices = full_data[
                    full_data.id.isin(real_train)
                ].index.to_list()
                real_val_indices = full_data[
                    full_data.id.isin(real_val)
                ].index.to_list()
                real_test_indices = full_data[
                    full_data.id.isin(real_test)
                ].index.to_list()
                new_indices.append(
                    (real_train_indices, real_val_indices, real_test_indices, label)
                )
            return new_indices
        ...
        def compute_overlap(a, b):
            ...
            result = [lower_to_orig_a[item] for item in overlap] + [
                lower_to_orig_b[item] for item in overlap
            ]
        ...
        def combine_for_multitest(dataset_indices, split_label):
            ...
            for i in range(5):
                ...
                idxs.append(
                    (tr_idxs, sorted(vl_idxs), ts_idxs, f"{split_label}_{i}")
                )  # sort to prevent python reordering
            return idxs
        ...
        def aggregate_multitest(data_label, seed, leaveout_method):
            if "dataset" not in data_label.columns:
                raise ValueError(
                    "The setting must be created such that different datasets are delimited."
                )
            ...
            for ds in data_label.dataset.unique():
                ...
                try:
                    ...
                    leave_x_out_indices = realign_indices(
                        data_label, sub_data, leave_x_out_indices
                    )
                    dataset_indices[ds] = leave_x_out_indices
                except (ValueError, NotImplementedError):
                    print(
                        f"{leaveout_method} does not work for {ds}, including all data"
                    )
                    dataset_indices[ds] = [
                        (sub_data.index.to_list(), [], [], "include_all_of_this")
                    ]
                    continue
        ...
        if leaveout_method == "leave_one_cl_out":
            ...
            for _, (train_indices, test_indices) in enumerate(
                ...
                train_indices, valid_indices = unseen_cl_validation(
                    train_indices, seed=seed
                )
                indices.append((train_indices, valid_indices, test_indices, cl_holdout))

                if (
                    _ > 15
                ):  # reduce for runtime in datasets with more than 15 cell lines
                    break
            ...
            for i, (_, test_idx) in enumerate(kf.split(cell_lines)):
                ...
                test_indices = data_label.index[
                    data_label["cell_line"].isin(cls_exclude)
                ].tolist()
                train_indices = list(np.setdiff1d(data_label.index, test_indices))
                train_indices, valid_indices = unseen_cl_validation(
                    train_indices, n=int(len(cell_lines) * 0.1), seed=seed
                )
                indices.append(
                    (
                        train_indices,
            ...
            tissue_dict = (
                ccle.groupby("lineage")["stripped_cell_line_name"].apply(list).to_dict()
            )
            ...
            for i, (_, test_idx) in enumerate(kf.split(all_tissues)):
                ...
                cls_exclude = [
                    cell_line
                    for tissue in tissues_exclude
                    for cell_line in tissue_dict[tissue]
                ]
                ...
                indices.append(
                    (train_indices, valid_indices, test_indices, f"tissue_split{i}")
                )
            ...
            leiden_dict = (
                ccle_obs.groupby("leiden")["stripped_cell_line_name"]
                .apply(list)
                .to_dict()
            )
            n_splits = 5
            ...
            for i, (_, test_idx) in enumerate(kf.split(all_clusters)):
                ...
                cls_exclude = [
                    cell_line
                    for cluster in clusters_exclude
                    for cell_line in leiden_dict[cluster]
                ]
                if len(set(cls_exclude) & set(cell_lines)) <= 3:
                    continue
                test_indices = data_label.index[
                ...
                indices.append(
                    (train_indices, valid_indices, test_indices, f"cluster_split{i}")
                )
            ...
            for i, (_, test_idx) in enumerate(kf.split(ivs)):
                ...
                train_indices = list(np.setdiff1d(data_label.index, test_indices))
                train_indices, valid_indices = unseen_ivs_validation(
                    train_indices, n=int(len(ivs) * 0.1), seed=seed
                )
                indices.append(
                    (
                        train_indices,
                        valid_indices,
                        test_indices,
                        f"iv_{i}_TrainedOn{len(_)}",
                    )
                )

        elif leaveout_method == "leave_ivs_fish":
            """A split specifically for the zebrafish dataset, where we only have 22 perturbations."""
            fish_ivs = np.setdiff1d(
                list(set(data_label["iv1"].unique()) | set(data_label["iv2"].unique())),
                ["negative_gene", "negative_drug"],
            )
            ...
            for i, (_, test_idx) in enumerate(kf.split(fish_ivs)):
                ...
                train_indices = list(np.setdiff1d(data_label.index, test_indices))
                train_indices, valid_indices = unseen_ivs_validation(
                    train_indices, n=int(len(fish_ivs) * 0.1), seed=seed
                )
                indices.append(
                    (
                        train_indices,
                        valid_indices,
                        test_indices,
                        f"iv_{ivs_exclude[0]}_TrainedOn{len(_)}",
                    )
                )
            ...
            scaffold_dict = (
                smiles_obs.groupby("scaffold")["smiles"].apply(list).to_dict()
            )
            ...
            for i, (_, test_idx) in enumerate(kf.split(all_scaffolds)):
                ...
                ivs_exclude = [
                    iv
                    for scaffold in scaffolds_exclude
                    for iv in scaffold_dict[scaffold]
                ]
                ...
                indices.append(
                    (train_indices, valid_indices, test_indices, f"scaffold_split{i}")
                )
            ...
            for i, (_, test_idx) in enumerate(kf.split(all_clusters)):
                clusters_exclude = np.array(all_clusters)[test_idx]
                ivs_exclude = [
                    iv for cluster in clusters_exclude for iv in leiden_dict[cluster]
                ]
                if len(set(ivs_exclude) & set(ivs)) <= 3:
                    continue
                test_indices = data_label.index[
                ...
                clusters_exclude = [
                    str(c) for c in clusters_exclude
                ]  # convert to str for join
                indices.append(
                    (train_indices, valid_indices, test_indices, f"cluster_split{i}")
                )
            ...
            leave_cl_out_indices = self.create_splits(
                sm_data, new_splitting_config, data_config
            )
            leave_cl_out_indices = realign_indices(
                data_label, sm_data, leave_cl_out_indices
            )
            for index in leave_cl_out_indices:
                indices.append(
                    (
                        list(index[0]) + train_indices,
                        index[1],
                        index[2],
                        f"wref_{index[3]}",
                    )
            ...
            leave_iv_out_indices = self.create_splits(
                sm_data, new_splitting_config, data_config
            )
            leave_iv_out_indices = realign_indices(
                data_label, sm_data, leave_iv_out_indices
            )
            ...
            if len(cell_lines) < 10:
                raise ValueError(
                    "Fewer than 10 cell lines, it is not recommended to run leave_more_cl_out"
                )
            ...
            for i, (_, test_idx) in enumerate(kf.split(cell_lines)):
                ...
                test_indices = data_label.index[
                    data_label[data_config["cl_col"]].isin(exclude)
                ].tolist()
                for frac in fracs:
                    include = _choose(
                        np.setdiff1d(cell_lines, exclude),
                        int(len(cell_lines) * frac),
                        seed=SEED,
                    )
                    train_indices = data_label.index[
                        data_label[data_config["cl_col"]].isin(include)
                    ].tolist()
                    remaining_indices = np.setdiff1d(
                        np.setdiff1d(data_label.index.tolist(), train_indices),
                        test_indices,
                    )
                    _, valid_indices = unseen_cl_validation(
                        remaining_indices, n=int(len(cell_lines) * 0.1), seed=seed
                    )
                    indices.append(
                        (
                            train_indices,
            ...
            for i, (_, test_idx) in enumerate(kf.split(ivs)):
                ...
                for frac in fracs:
                    ivs_include = _choose(
                        np.setdiff1d(ivs, ivs_exclude), int(len(ivs) * frac), seed=SEED
                    )
                    train_indices = data_label.index[
                        data_label["iv1"].isin(ivs_include)
                        & data_label["iv2"].isin(ivs_include)
                    ].tolist()
                    if (
                        len(train_indices) <= 100
                    ):  # in sparsely complete combinations, this can end up exceedingly small
                        continue
                    remaining_indices = np.setdiff1d(
                        np.setdiff1d(data_label.index.tolist(), train_indices),
                        test_indices,
                    )
                    _, valid_indices = unseen_ivs_validation(
                        remaining_indices, n=int(len(ivs) * 0.1), seed=seed
                    )
                    indices.append(
                        (
                            train_indices,
            ...
            commons = compute_overlap(
                sm_data["cell_line"].unique(), bg_data["cell_line"].unique()
            )
            print(f"{len(commons)} in common (including dups)")
            sm_train_fixed = sm_data[
                ~sm_data["cell_line"].isin(commons)
            ].index.to_list()
            print(
                f"small and large always included in train: {len(sm_train_fixed)}, {bg_data.shape[0]}"
            )

            sm_data["id"] = list(range(sm_data.shape[0]))
            subset = sm_data[sm_data["cell_line"].isin(commons)]
            print(
                f"size of overlapping portion from which to select val/test: {subset.shape[0]}"
            )
            new_splitting_config = splitting_config.copy()
            new_splitting_config["method"] = "leave_more_cl_out"
            leave_cl_out_indices = self.create_splits(
                subset, new_splitting_config, data_config
            )
            leave_cl_out_indices = realign_indices(
                sm_data, subset, leave_cl_out_indices
            )
            for index in leave_cl_out_indices:
                train, val, test, label = index
                for expanded_dataset in [True, False]:
                    ...
                    indices.append(
                        (
                            ...
                            val,
                            test,
                            f"{label}_withref_{expanded_dataset}",
                        )
                    )
            ...
            sm_train_fixed = sm_train_fixed[
                ~sm_train_fixed["iv2"].isin(commons)
            ].index.to_list()
            print(
                f"small and large always included in train: {len(sm_train_fixed)}, {bg_data.shape[0]}"
            )
            ...
            sm_data["id"] = list(range(sm_data.shape[0]))
            subset = sm_data[
                sm_data["iv1"].isin(commons) | sm_data["iv2"].isin(commons)
            ]
            print(
                f"size of overlapping portion from which to select val/test: {subset.shape[0]}"
            )
            ...
            leave_iv_out_indices = realign_indices(
                sm_data, subset, leave_iv_out_indices
            )
            for index in leave_iv_out_indices:
                train, val, test, label = index
                for expanded_dataset in [True, False]:
            ...
            new_splitting_config["method"] = "leave_iv_out"
            leave_iv_out_indices = self.create_splits(
                data_label, new_splitting_config, data_config
            )
            new_splitting_config = splitting_config.copy()
            new_splitting_config["method"] = "leave_cl_out"
            leave_cl_out_indices = self.create_splits(
            ...
            for i in range(5):
                ...
                indices.append(
                    (train_indices, val_indices, test_indices, f"bothsplit_{i}")
                )
            ...
            for ds in data_label.dataset.unique():
                ...
                try:
                    ...
                    new_splitting_config["method"] = "leave_cl_out"
                    leave_cl_out_indices = self.create_splits(
                        sub_data, new_splitting_config, data_config
                    )
                    leave_cl_out_indices = realign_indices(
                        data_label, sub_data, leave_cl_out_indices
                    )
            ...
            for ds in data_label.dataset.unique():
                ...
                leave_iv_out_indices = realign_indices(
                    data_label, sub_data, leave_iv_out_indices
                )
            ...
            dataset_indices = aggregate_multitest(
                data_label, seed, leaveout_method.split("_multitest")[0]
            )
        ...
        if len(indices) == 0:
            raise ValueError(
                f"Stratification {leaveout_method} is invalid, likely not enough categories"
            )
        ...
        for split in indices:
            ...
            if type(split[2]) == dict:
                test = split[2]["all"]
            if (
                len(set(split[0]) & set(split[1])) > 0
                or len(set(split[0]) & set(test)) > 0
                or len(set(split[1]) & set(test)) > 0
            ):
                raise NotImplementedError(
                    f"leavout method {leaveout_method} has overlapping indices!!"
                )
            if len(split[0]) < 1 or len(split[1]) < 1 or len(test) < 1:
                raise NotImplementedError(
                    f"leavout method {leaveout_method} has no examples in one of train:{len(split[0])}, val:{len(split[1])}, test:{len(test)} with description {split[3]}"
                )
        ...
        final_splits = [
            [
                data_label.loc[split[0]],
                data_label.loc[split[1]],
                data_label.loc[split[2]],
                split[3],
            ]
            for split in indices
        ]
