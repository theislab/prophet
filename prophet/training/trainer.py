...
def train_transformer(data, model, config, name, seed):
    ...
    model_checkpointer = ModelCheckpoint(
        dirpath=dirpath, save_top_k=1, every_n_epochs=1, monitor="R2", mode="max"
    )
    r2_callback = R2ScoreCallback(
        device=model.device, average=True if config.setting == "everything" else False
    )
    early_stopping = EarlyStopping(
        monitor="R2", mode="max", patience=config.patience, min_delta=0.0
    )
    ...
    if config.ckpt_path is not None:
        ...
        if config.fine_tune:
            ...
            model = TransformerPredictor.load_from_checkpoint(
                ckpt_file, warmup=config.transformer.warmup
            )
            trainer.fit(
                model=model,
