...
class ProphetTrainer:
    ...
    def load_split_assignments(self, checkpoint_path: str) -> Optional[Dict]:
        ...
        checkpoint_dir = Path(checkpoint_path).parent if Path(checkpoint_path).is_file() else Path(checkpoint_path)
    ...
    def create_new_split_assignments(self, df: pd.DataFrame, iv_cols: List[str], 
                                   seed: int, n_splits: int, val_fraction: float) -> Dict:
        ...
        for i in range(n_splits):
            ...
            for phenotype, interventions in phenotype_interventions.items():
                ...
                if len(interventions) > 100:  # Only show details for larger phenotypes
                    print(f"    {phenotype} fold {i}: {len(train_ivs)} train, {len(val_ivs)} val, {len(test_ivs)} test")
            ...
            print(f"  🎯 Total fold {i}: {len(all_train_interventions)} train, {len(all_val_interventions)} val, {len(all_test_interventions)} test interventions")
    ...
    def create_splits(self, df: pd.DataFrame, splitting_config: dict, data_config: dict) -> List[Tuple]:
        ...
        if method == "random":
            ...
            for i in range(n_splits):
                ...
                print(f"  Assigned interventions - Train: {len(train_interventions)}, Val: {len(val_interventions)}, Test: {len(test_interventions)}")
                print(f"  Present interventions - Train: {len(current_train_ivs)}, Val: {len(current_val_ivs)}, Test: {len(current_test_ivs)}")
        ...
        if splits and hasattr(self, 'split_assignments') and self.split_assignments:
            ...
            print(f"  ASSIGNED: Train: {len(assigned_train)}, Val: {len(assigned_val)}, Test: {len(assigned_test)}")
            print(f"  ACTUAL: Train: {len(actual_train_interventions)}, Val: {len(actual_val_interventions)}, Test: {len(actual_test_interventions)}")
            print(f"  Validation-only conditions: {len(val_only)} (examples: {sorted(list(val_only))[:5]})")
            print(f"  Test-only conditions: {len(test_only)} (examples: {sorted(list(test_only))[:5]})")
            print()
    ...
    def run_cross_validation(self, splits: List[Tuple], data_config: dict, 
        ...
        model_split_path = Path(output_dir) / data_config['setting'] / self.config['splitting']['method'] / str(self.seed)
