...
def mock_data():
    ...
    }, index=['gene1', 'gene2', 'drug1', 'negative_gene', 'negative_drug'])   # assume embedding for drug2 is not exist
...
def test_dataloader_phenotypes(mock_data):
    ...
    iv_embedding, cell_lines_embedding, phenotype_embedding, data_label, index = mock_data
    ...
    train_dataloader, valid_dataloader, test_dataloader, train_indices, test_indices, cl_holdout = dataloader_phenotypes(
...
def test_process_priors(mock_data):
    ...
    if phenotype_embedding is not None: #create dummy files for testing of process_priors
...
def test_remove_nonexistent_cat(mock_data):
    ...
    for i, embedding in enumerate(embeddings):
        if embedding is None:  # phenotype embedding can be None
            continue
        data_label = remove_nonexistent_cat(data_label, embedding, cols[i], verbose=True)
  
    assert len(data_label) == 3  # 4-1= 3 According to mock embeddings and  datalabel, one row contains
